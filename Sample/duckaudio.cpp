...
int DuckAudio::displayParaID(int paraID , int parameter_length , int &flag)
{
    if(paraID == 0 && parameter_length-kParameterHeaderLength == kSizeofDouble)
    {
        std::cout << "参数名称是DurationSecs" << std::endl;
        flag++;
        return 0;
    }
    else if(paraID == 1 && parameter_length-kParameterHeaderLength == kSizeofDouble)
    {
        std::cout << "参数名称是Volume" << std::endl;
        flag++;
        return 0;
    }
...
void DuckAudio::displayData(int paraID,char paradata)
{
    if(paraID == 0)
    {
...
vector<CParameter> DuckAudio::ParameterAnalysis(vector<uint8> receive_package,vector<vector<uint8>>& res_package,vector<CParameter>& parameter,CMessage& message)
{
    int flag = 0;
