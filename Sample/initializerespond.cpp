...
InitializeRespond::InitializeRespond()
        :MessageBase(messageID,paranum)
{
}

int InitializeRespond::displayParaID(int paraID , int parameter_length , int &flag)
{
    if(paraID == 0 && parameter_length-kParameterHeaderLength == kSizeofChar)
    {
        std::cout << "参数名称是RightHand" << std::endl;
        flag++;
        return 0;
    }
...
void InitializeRespond::displayData(int paraID,char paradata)
{
    if(paraID == 0)
    {
...
vector<CParameter> InitializeRespond::ParameterAnalysis(vector<uint8> receive_package,vector<vector<uint8>>& res_package,vector<CParameter>& parameter,CMessage& message)
{
    int flag = 0;
