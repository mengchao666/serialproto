...
RequestButtonKey::RequestButtonKey()
        :MessageBase(messageID,paranum)
{
}

int RequestButtonKey::displayParaID(int paraID , int parameter_length , int &flag)
{
    if(paraID == 0)
    {
        std::cout << "参数名称是PlayPause" << std::endl;
        return 0;
    }
    else if(paraID == 1)
    {
        std::cout << "参数名称是ScanNextTrack" << std::endl;
        return 0;
    }
    else if(paraID == 2)
    {
        std::cout << "参数名称是ScanPreviousTrack" << std::endl;
        return 0;
    }
    else if(paraID == 3 )
    {
        std::cout << "参数名称是Play" << std::endl;
        flag++;
        return 0;
    }
    else if(paraID == 4 )
    {
        std::cout << "参数名称是Pause" << std::endl;
        flag++;
        return 0;
    }
    else if(paraID == 5)
    {
        std::cout << "ACHome值" << std::endl;
        flag++;
        return 0;
    }
    else if(paraID == 6)
    {
        std::cout << "ACBack值" << std::endl;
        flag++;
        return 0;
    }
    else
...
void RequestButtonKey::displayData(int paraID,char paradata)
{
    if(paraID == 0)
    {
...
vector<CParameter> RequestButtonKey::ParameterAnalysis(vector<uint8> receive_package,vector<vector<uint8>>& res_package,vector<CParameter>& parameter,CMessage& message)
{
    int flag = 0;
