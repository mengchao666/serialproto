...
RequestPhoneKey::RequestPhoneKey()
        :MessageBase(messageID,paranum)
{
}

int RequestPhoneKey::displayParaID(int paraID , int parameter_length , int &flag)
{
}

void RequestPhoneKey::displayData(int paraID,char paradata)
{


}

vector<CParameter> RequestPhoneKey::ParameterAnalysis(vector<uint8> receive_package,vector<vector<uint8>>& res_package,vector<CParameter>& parameter,CMessage& message)
{
    int flag = 0;
