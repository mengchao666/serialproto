...
StreamsStateChanged::StreamsStateChanged()
        :MessageBase(messageID,paranum)
{
}

int StreamsStateChanged::displayParaID(int paraID , int parameter_length , int &flag)
{
    if(paraID == 0 && parameter_length-kParameterHeaderLength == kSizeofChar)
    {
        std::cout << "参数名称是MainAudio主音频流是否连接" << std::endl;
        flag++;
        return 0;
    }
    else if(paraID == 1 && parameter_length-kParameterHeaderLength == kSizeofChar)
    {
        std::cout << "参数名称是AltAudio警告音频流是否连接" << std::endl;
        flag++;
        return 0;
    }
    else if(paraID == 2 && parameter_length-kParameterHeaderLength == kSizeofChar)
    {
        std::cout << "参数名称是MainScreen主视频流是否连接" << std::endl;
        flag++;
        return 0;
    }
...
void StreamsStateChanged::displayData(int paraID,char paradata)
{
    if(paraID == 0)
    {
...
vector<CParameter> StreamsStateChanged::ParameterAnalysis(vector<uint8> receive_package,vector<vector<uint8>>& res_package,vector<CParameter>& parameter,CMessage& message)
{
    int flag = 0;
