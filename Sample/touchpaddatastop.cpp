...
TouchpadDataStop::TouchpadDataStop()
        :MessageBase(messageID,paranum)
{
}

int TouchpadDataStop::displayParaID(int paraID , int parameter_length , int &flag)
{
    if(paraID == 0)
    {
...
void TouchpadDataStop::displayData(int paraID,char paradata)
{
    if(paraID == 0)
    {
...
vector<CParameter> TouchpadDataStop::ParameterAnalysis(vector<uint8> receive_package,vector<vector<uint8>>& res_package,vector<CParameter>& parameter,CMessage& message)
{
    int flag = 0;
