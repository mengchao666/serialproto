...
UnduckAudio::UnduckAudio()
        :MessageBase(messageID,paranum)
{
}

int UnduckAudio::displayParaID(int paraID , int parameter_length , int &flag)
{
...
void UnduckAudio::displayData(int paraID,char paradata)
{
    if(paraID == 0)
    {
...
vector<CParameter> UnduckAudio::ParameterAnalysis(vector<uint8> receive_package,vector<vector<uint8>>& res_package,vector<CParameter>& parameter,CMessage& message)
{
    int flag = 0;
